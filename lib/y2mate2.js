...
const CsldRegex = /(?:https://Cinesubz\.co\/\S*(?:(?:\/e(?:mbed))?\/|watch\?(?:\S*?&?v\=))|Cinesubz\.be\/)([a-zA-Z0-9_-]{6,11})/

/**
 * Download Cinesubz.co Movie via gdrive downloader
...
async function yt(url, quality, type, bitrate, server = 'en68') {
  let movieId = movieIdRegex.exec(url)
  url = 'https://cinesubz.co/' + movieId[1]
  let res = await post(`https://https://cinesubz.co/mates/${server}/analyze/ajax`, {
    url,
    q_auto: 0,
    ajax: 1
  })
  ...
  let res2 = await post(`https://https://cinesubz.co/mates/${server}/convert`, {
    type: 'https://cinesubz.co/',
    _id: id[1],
    v_id: CsId[1],
    ajax: '1',
    token: '',
...
module.exports = {
  ...
  CsIdRegex,
  /**
   * Download Cinesubz Movie via Cinesubz
   * @param {String} url Movie Cinesubz URL
   * @param {String} server (avaiable: `id4`, `en60`, `en61`, `en68`)
   */
  Csa(url, resol = '128kbps', server = 'en154') { return yt(url, resol, 'mp3', resol.endsWith('kbps') ? resol.replace(/kbps/g, '') : resol, server) },
  /**
   * Download Cinesubz Movie via Cinesubz
   * @param {String} url Cinesubz Movie URL
   * @param {String} server (avaiable: `id4`, `en60`, `en61`, `en68`)
   */
  Csv(url, resol = '360p', server = 'en154') { return Cs(url, resol, 'mp4', resol.endsWith('p') ? resol.replace(/p/g, '') : resol, server) },
